    ...
    <div className="space-y-12">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {vehicle.specs.map((spec) => (
            <div key={spec.label} className="rounded-xl p-4 border" style={{ backgroundColor: '#161b27', borderColor: '#2a3347' }}>
      ...
      <section>
        ...
        <div className="space-y-3">
          {providers.map((p) => (
            <div key={p.name} className="rounded-xl p-4 border flex flex-col sm:flex-row sm:items-center gap-3" style={{ backgroundColor: '#161b27', borderColor: '#2a3347' }}>
              ...
              <div className="flex gap-3 text-xs font-mono">
                {p.ac && (
                  <span className="px-2 py-0.5 rounded" style={{ backgroundColor: '#1e2535', color: '#60a5fa' }}>
                    AC {p.ac}
                  </span>
                )}
                {p.dc && (
                  <span className="px-2 py-0.5 rounded" style={{ backgroundColor: '#1e2535', color: '#22c55e' }}>
                    DC {p.dc}
                  </span>
                )}
              </div>
